...
export function Tabs<T extends string>({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const nextKeys = isVert ? ["ArrowDown", "ArrowRight"] : ["ArrowRight"];
    const prevKeys = isVert ? ["ArrowUp", "ArrowLeft"] : ["ArrowLeft"];
  ...
  return (
    <div
      className={`tabs ${isVertical ? "tabs--vertical" : "tabs--horizontal"} ${className}`}
      role="tablist"
      onKeyDown={handleKeyDown}
      style={{
        flexDirection: isVertical ? "column" : "row",
        borderBottom: isVertical ? "none" : "1px solid var(--border-dim)",
        borderRight: isVertical ? "1px solid var(--border-dim)" : "none",
        height: isVertical ? "100%" : undefined,
        width: isVertical ? "40px" : undefined,
        alignItems: "center",
        justifyContent: isVertical ? "flex-start" : "center",
        padding: isVertical ? "6px 2px" : "2px",
        ...style,
      }}
    >
      {tabs.map((tab) => (
...
export function Dialog({
  ...
  useEffect(() => {
    ...
    if (focusables.length > 0) {
      const promptInput = focusables.find((el) =>
        el.classList.contains("modal-prompt-input"),
      );
      if (promptInput) {
        promptInput.focus();
      } else {
...
export function NewDocumentModal({
  ...
  const [docType, setDocType] = useState<"document" | "slides" | "graphic">(
    "document",
  );
  const [pageSizePreset, setPageSizePreset] = useState<
    "A4" | "Letter" | "Legal" | "Slide16_9" | "Custom"
  >("A4");
  const [orientation, setOrientation] = useState<"portrait" | "landscape">(
    "portrait",
  );
  const [marginPreset, setMarginPreset] = useState<
    "normal" | "narrow" | "wide" | "none" | "custom"
  >("normal");
  ...
  const handlePageSizePresetChange = (
    preset: "A4" | "Letter" | "Legal" | "Slide16_9" | "Custom",
  ) => {
  ...
  const handleMarginPresetChange = (
    preset: "normal" | "narrow" | "wide" | "none" | "custom",
  ) => {
  ...
  return createPortal(
    <div className="modal-overlay" onClick={onCancel}>
      ...
      >
        <div
          className="new-doc-modal__header"
          style={{
            display: "flex",
            alignItems: "center",
            gap: 10,
        ...
        {step === 1 ? (
          ...
          >
            <div className="field-grid">
              ...
              <Field label="Document Type">
                <select
                  ...
                  onChange={(e) =>
                    handleDocTypeChange(
                      e.target.value as "document" | "slides" | "graphic",
                    )
                  }
                >
                  <option value="document">
                    📄 Standard Document (Report, Essay)
                  </option>
                  <option value="slides">
                    🖼️ Slide Presentation (Pitch Deck, Slide Deck)
                  </option>
                  <option value="graphic">
                    🎨 Visual Graphic (Infographic, Newsletter)
                  </option>
                </select>
              </Field>
            </div>
            ...
            <div className="field-grid">
              ...
              <Field label="Background Color">
                <input
                  ...
                  style={{ padding: 0, height: 28, cursor: "pointer" }}
                />
              </Field>
            </div>
            ...
            <Field
              ...
              hint="Describe layout, headers, content details, colors..."
            >
              <textarea
                value={instructions}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <p style={{ margin: 0, fontSize: "11px", color: "var(--text-2)" }}>
              Copy the prompt below and paste it to your AI coding agent (e.g.
              Antigravity) to build the page structure and contents.
            </p>
...
interface HorizontalScrollContainerProps {
  ...
  className?: string;
  style?: React.CSSProperties;
  contentAlignItems?: React.CSSProperties["alignItems"];
  navigation?: "arrows" | "scrollbar";
}
...
export function HorizontalScrollContainer({
  ...
  const useArrows = navigation !== "scrollbar";
  ...
  return (
    ...
    >
      {useArrows && showLeftArrow && (
        <button
          type="button"
          className="scroll-arrow-btn scroll-arrow-btn--left"
          onClick={() => scroll("left")}
          title="Scroll Left"
          style={{
            position: "absolute",
      ...
      <div
        ref={scrollRef}
        className={navigation === "scrollbar" ? undefined : "hide-scrollbar"}
        style={{
          display: "flex",
          flexDirection: "row",
      ...
      {useArrows && showRightArrow && (
        <button
          type="button"
          className="scroll-arrow-btn scroll-arrow-btn--right"
